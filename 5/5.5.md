...
Go uses *[buildr](http://buildr.apache.org/)* for its build infrastructure except for generating mac installers which is done through *[ant](http://ant.apache.org/)*. 
...
### <a name="modules"></a>5.5.1.1 Modules
There are 2 components in Go: Server & Agent.

Config: 
* The code related to config that is required by both Server & Agent (like object mapping of config etc.) is in **'config-api'** module.
* While the code related to config that is required by Server alone (like config read, write etc.) is in **'config-server'** module.

The plugin infrastructure:
* **'go-plugin-api'** contains API exposed for plugin authors. is required by plugins during compile time.
...
Server:
* The complete code related to server is in **'server'** module. Contains code related to database, security, view, etc. 
The server webapp is created using rake tasks. We generate help docs, minify css, js & inline rails partials as part of this.
* The module **'server-launcher'** is a module definition with dependencies specified in pom file which is used in creation of launcher using buildr scripts.

Others:
* The code that is common to 'server' & 'agent' modules is in **'common'** module.
* The code that is common to all the modules is in **'base'** module.
* Code related to TFS implementation is a separate module **'tfs-impl'**. It was made into a separate module to resolve classpath issues.
...
### <a name="documentation-creation"></a>5.5.1.2 Documentation Creation
...
### <a name="pom-conventions"></a>5.5.1.3 Pom conventions
* Version of all Go modules is fixed to 1.0 which means the code for every module assumes that the latest code of its dependencies is available at version 1.0 in local maven repository (~/.m2/repository).

* The code does not follow standard maven structure. We have code in src/, resources is in resources/, test code is in test/ & test resources is in test-resources/ folder. The locations are specified in pom file to override defaults.

* maven-compiler-plugin is configured with source & target to be 1.6.

* All source dependencies are listed followed by test dependencies. In both source & test dependencies the Go modules are listed first followed by external library dependencies. We have tried our best to not redeclare dependencies that are available as transitive dependency from other modules / libraries. We have also tried to exclude transitive dependencies that are of a older / newer version than the ones that are required. We have a pipeline dedicated to watch packaged dependencies & fail whenever there is a change to it. This acts as a manual gate to make sure the dependency change was intended.

### <a name="jar-packaging"></a>5.5.1.4 Jar packaging
...
### <a name="tlb-integration"></a>5.5.1.5 [TLB](http://test-load-balancer.github.io/) integration
Modules 'common' & 'server' have a lot of tests. We use TLB to run them in parallel during builds.
...
The classpath is provided by maven to maintain consistency with 'maven-surefire-plugin'. So any test would run under same environment when run through maven / through TLB.

## <a name="installer-creation"></a>5.5.2 Installer Creation
...
### <a name="debian"></a>5.5.2.3 Debian
...
### <a name="windows"></a>5.5.2.4 Windows
...
### <a name="mac-osx"></a>5.5.2.5 Mac OS X
...
### <a name="solaris"></a>5.5.2.6 Solaris
